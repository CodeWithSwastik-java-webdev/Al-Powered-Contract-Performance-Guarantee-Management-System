...
const api = axios.create({ baseURL: import.meta.env.VITE_API_URL || '/api/v1', headers: { 'Content-Type': 'application/json' }, timeout: 15_000 })
...
api.interceptors.response.use((response) => response, (error) => {
  if (error.response) return Promise.reject(new Error(error.response.data?.error?.message ?? error.response.data?.message ?? 'An unexpected error occurred'))
